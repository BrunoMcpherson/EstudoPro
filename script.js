...
let dadosEstudo = {
    ...
    perfil: { nome: "", idade: "", telefone: "", email: "" } // Novos dados de cadastro
};
...
function alternarAuth(tela) {
    ...
    document.getElementById('auth-mensagem').classList.add('hidden'); // Limpa os erros

    if(tela === 'cadastro') {
        login.classList.add('hidden'); cadastro.classList.remove('hidden'); sub.innerText = "Crie sua conta para salvar na nuvem";
    } else {
        cadastro.classList.add('hidden'); login.classList.remove('hidden'); sub.innerText = "Acesse sua conta";
    }
}
...
async function fazerLogin() {
    ...
    catch (error) { exibirMensagemErro("Email ou senha incorretos."); }
}
...
async function criarConta() {
    ...
    if(!nome || !idade || !telefone || !email || !senha) return exibirMensagemErro("Preencha todos os campos do cadastro.");
    ...
    try {
        ...
        // Antes de mostrar a tela, já injeta os dados do perfil dele no banco de dados zerado
        dadosEstudo.perfil = { nome, idade, telefone, email };
        await setDoc(doc(db, "usuarios", user.uid), dadosEstudo);
    } catch (error) {
        exibirMensagemErro("Erro ao criar conta. Email já existe ou formato inválido.");
    }
}
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        // AQUI ESTÁ A CORREÇÃO DO LAYOUT (DE 'flex' PARA 'block')!
...
function checarOfensivaOnLoad() {
    ...
    if (ult && ult !== hjStr) { const diffDays = Math.ceil(Math.abs(parseDateLocal(hjStr) - parseDateLocal(ult)) / (1000 * 60 * 60 * 24)); if (diffDays > 1) { dadosEstudo.ofensiva.dias = 0; salvarLocal(false); } }
}
...
function adicionarConcurso() { const nome = document.getElementById('novo-concurso-nome').value.trim(); const data = document.getElementById('novo-concurso-data').value; if(nome && data) { const id = Date.now(); dadosEstudo.concursos.push({ id, nome, data }); dadosEstudo.concursoAtivo = id; document.getElementById('novo-concurso-nome').value = ''; document.getElementById('novo-concurso-data').value = ''; esconderFormConcurso(); salvarLocal(true); } else alert('Preencha nome e data.'); }
function mudarConcursoAtivo() { dadosEstudo.concursoAtivo = parseInt(document.getElementById('select-concurso').value); salvarLocal(false); }
function deletarConcursoAtivo() { if(dadosEstudo.concursoAtivo && confirm("Deseja apagar este concurso da lista?")) { dadosEstudo.concursos = dadosEstudo.concursos.filter(c => c.id !== dadosEstudo.concursoAtivo); dadosEstudo.concursoAtivo = dadosEstudo.concursos.length > 0 ? dadosEstudo.concursos[0].id : null; salvarLocal(true); } }
function calcularDiasRestantes(dataStr) { if(!dataStr) return "--"; const dp = new Date(dataStr); dp.setHours(23, 59, 59); const diff = dp - new Date(); return diff < 0 ? "0" : Math.ceil(diff / (1000 * 60 * 60 * 24)); }
...
function iniciarPomodoro(isFoco) {
    if (pomoRodando) return; pomoEmFoco = isFoco; const tFoco = (parseInt(document.getElementById('cfg-pomo-foco').value) || 25) * 60; const tPausa = (parseInt(document.getElementById('cfg-pomo-pausa').value) || 5) * 60;
    if(pomoTempoRestante === 0 || (pomoTempoRestante === tFoco) || (pomoTempoRestante === tPausa)) { pomoTempoRestante = isFoco ? tFoco : tPausa; }
    pomoRodando = true; pomoInterval = setInterval(() => {
        if (pomoTempoRestante > 0) { pomoTempoRestante--; atualizarDisplayPomo(); } 
        else { pararPomodoro(); tocarAlarme(); if(pomoEmFoco) { const min = parseInt(document.getElementById('cfg-pomo-foco').value) || 25; const matId = document.getElementById('select-materia-tempo').value; dadosEstudo.tempoTotal += min; dadosEstudo.pomodorosRealizados++; registrarAtividade(min, 0, 0, 0, matId); salvarLocal(true); setTimeout(() => alert("Foco Finalizado!"), 500); } else setTimeout(() => alert("Pausa Finalizada!"), 500); }
    }, 1000);
}
...
function resetarTempo() { if(confirm("Zerar todo histórico de tempo?")) { dadosEstudo.tempoTotal = 0; dadosEstudo.materias.forEach(m => m.tempo = 0); Object.keys(dadosEstudo.historicoDias).forEach(d => { dadosEstudo.historicoDias[d].minutos = 0; }); salvarLocal(true); } }
...
function alternarConclusao(iMat, iSub) { const sub = dadosEstudo.materias[iMat].subtemas[iSub]; sub.concluido = !sub.concluido; if (sub.concluido) { agendarRevisoes(dadosEstudo.materias[iMat].nome, sub.nome); registrarAtividade(0,0,0,0); } salvarLocal(false); }
function deletarMateria(i) { if (confirm("Excluir matéria?")) { let mat = dadosEstudo.materias[i]; if(mat.questoes) { dadosEstudo.questoesGerais.acertos = Math.max(0, dadosEstudo.questoesGerais.acertos - mat.questoes.acertos); dadosEstudo.questoesGerais.erros = Math.max(0, dadosEstudo.questoesGerais.erros - mat.questoes.erros); } if(mat.tempo) dadosEstudo.tempoTotal = Math.max(0, dadosEstudo.tempoTotal - mat.tempo); dadosEstudo.materias.splice(i, 1); salvarLocal(true); } }
...
function renderizarTopBar() {
    ...
    if(dadosEstudo.concursoAtivo) { selectConc.value = dadosEstudo.concursoAtivo; const conc = dadosEstudo.concursos.find(c => c.id === dadosEstudo.concursoAtivo); if(conc) { const dp = conc.data.split('-'); dataDisp.innerText = `${dp[2]}/${dp[1]}/${dp[0]}`; diasDisp.innerText = calcularDiasRestantes(conc.data); } } else { dataDisp.innerText = '--/--/----'; diasDisp.innerText = '--'; }
...
function renderizarEdital() {
    ...
    dadosEstudo.materias.forEach((mat, iMat) => {
        ...
        let subs = mat.subtemas.map((s, iSub) => `<div class="flex items-center gap-2 mt-2 ml-4 p-1 hover:bg-gray-50 rounded"><input type="checkbox" ${s.concluido ? 'checked' : ''} onchange="alternarConclusao(${iMat}, ${iSub})" class="w-4 h-4 text-indigo-600 cursor-pointer"><span class="${s.concluido ? 'line-through text-gray-400' : 'text-gray-700'} text-sm">${s.nome}</span></div>`).join('');
...
function atualizarGraficos() {
    ...
    dadosEstudo.materias.forEach(m => { if(m.questoes && (m.questoes.acertos > 0 || m.questoes.erros > 0)) { labelsM.push(m.nome.substring(0, 10)); const pct = Math.round((m.questoes.acertos / (m.questoes.acertos + m.questoes.erros)) * 100); dadosM.push(pct); coresM.push(pct >= 80 ? '#10b981' : (pct >= 60 ? '#f59e0b' : '#ef4444')); } if(m.tempo && m.tempo > 0) { labelsT.push(m.nome.substring(0, 10)); dadosT.push((m.tempo / 60).toFixed(1)); } });
